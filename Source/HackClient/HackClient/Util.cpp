...
void SplashScreen(CSplash* ClassInst, int SplashType, bool TextSwitch, char DisplayText[255], int CloseTime)
{
	...
	switch(SplashType)
	{
		case 0:
			ResId = IDB_BITMAP1;
			TextColor = RGB(52,109,175);
			break;
		case 1:
			ResId = IDB_BITMAP2;
			TextColor = RGB(179,105,53);
			break;
		case 2:
			ResId = IDB_BITMAP3;
			TextColor = RGB(175,52,52);
			break;
	}
...
char* GetProcess(int& count) {
	...
	char* concatenatedNames = new char[maxProcesses * MAX_PATH];
	concatenatedNames[0] = '\0';  // Ensure the initial string is empty
	count = 0;

	if (Process32First(snapshot, &processEntry)) {
		do {
			// Concatenate the process name with periods (.)
			strcat(concatenatedNames, processEntry.szExeFile);
			strcat(concatenatedNames, ",");
