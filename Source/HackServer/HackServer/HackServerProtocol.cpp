...
void CHCProcessInfoRecv(SHDP_SHOW_PROCESS_SEND* lpMsg, int index) {
	try {
		gClientManager[index].SetProcess(lpMsg->processes);
	}
	catch (...) {
...
void HackServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x03:
			...
			break;
		case 0x04:
			CHCProcessInfoRecv((SHDP_SHOW_PROCESS_SEND*)lpMsg, index);
			break;
		default:
