...
void CScreenShow::DrawScreen(HWND hWnd, HDC hdc)
{
	...
	image.Draw(hdc,0,0,gScreenShow.ScreenWidth,gScreenShow.ScreenHeight,0,0,gScreenShow.ScreenWidth,gScreenShow.ScreenHeight+gScreenShow.ScreenDiff); //Cambiar tamaño de imagen, pero no del contenedor
...
void CScreenShow::RecvScreenData(PMSG_SCREENDATA_RECV* lpMsg, int aIndex) // Tamaño de ventana
{
	...
	int Horres = GetDeviceCaps(hDCScreen,  HORZRES); //TAMAÑO DE VENTANA PERO NO DE IMAGEN
	int Vertres = GetDeviceCaps(hDCScreen,  VERTRES); //TAMAÑO DE VENTANA PERO NO DE IMAGEN
	...
	else
	{
		...
		this->ScreenWidth = Horres; //Tamaño completo de la ventana
		this->ScreenHeight = Vertres; //Tamaño completo de la ventana
		this->ScreenDiff = 100;
	}
