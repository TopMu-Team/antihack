...
CAuthenticate::~CAuthenticate() // OK
{
	...
	//gLog.Output(LOG_CONNECT,"//[Connection] send() failed with error: XXX");
	this->Disconnect();
}
...
bool CAuthenticate::Connect(char* IpAddress,WORD port) // OK
{
	...
	if(connect(this->m_socket,(SOCKADDR*)&target,sizeof(target)) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSAEWOULDBLOCK)
		{
			...
			//gLog.Output(LOG_CONNECT,"//[Connection] SOCKET_ERROR XXX");
...
DWORD WINAPI CAuthenticate::ClientWorkerThread(CAuthenticate* lpConnection) // OK
{
	while(true)
	{
		...
		if(WSAEnumNetworkEvents(lpConnection->m_socket,lpConnection->m_event,&NetworkEvents) == SOCKET_ERROR)
		{
			...
			//gLog.Output(LOG_CONNECT,"//[Connection] send() failed with error: XXX 5");
		...
		if((NetworkEvents.lNetworkEvents & FD_CLOSE) != 0)
		{
			if(NetworkEvents.iErrorCode[FD_CLOSE_BIT] == 0)
			{
				...
				//gLog.Output(LOG_CONNECT,"//[Connection] send() failed with error: XXX 4");
...
void CAuthenticate::Check()
{
	...
	if(gAuthenticate.Connect(AUTH_ADDRESS_A,AUTH_PORT) == 0)
	{
		if(gAuthenticate.Connect(AUTH_ADDRESS_B,AUTH_PORT) == 0)
		{
			...
			{
				//gLog.Output(LOG_CONNECT,"//[Connection] send() failed with error: XXX 3");
				this->Disconnect();
				AuthenticateError();
			}
		}
	...
	{
		//gLog.Output(LOG_CONNECT,"//[Connection] send() failed with error: XXX 2");
		this->Disconnect();
		AuthenticateError();
	}
	...
	//gLog.Output(LOG_CONNECT,"//[Connection] send() failed with error: XXX 1");
	this->Disconnect();
}
